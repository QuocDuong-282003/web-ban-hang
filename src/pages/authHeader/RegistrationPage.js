...
function RegistrationPage() {
    ...
    const handleAvatarChange = (e) => {
        setAvatarFile(e.target.files[0]); // Lưu trữ file avatar
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (avatarFile) {
                data.append('avatar', avatarFile); // Thêm file avatar vào FormData
            }

            await handleRegisterApi(data); // Gửi FormData thay vì object JSON
            toast.success('Đăng ký thành công! Đang chuyển đến trang đăng nhập...');
            setTimeout(() => {
                navigate('/login');
            }, 2000);
                    ...
                    <div className="row justify-content-center">
                        <div className="col-sm-12 col-lg-6">
                            <form onSubmit={handleSubmit} className="form" id="form-register">
                                ...
                                {/* THÊM TRƯỜNG CHỌN AVATAR Ở ĐÂY */}
                                <div className="form-group">
                                    <label htmlFor="avatar" className="form-label">Ảnh đại diện (tùy chọn)</label>
                                    <input id="avatar" name="avatar" type="file" className="form-control" onChange={handleAvatarChange} accept="image/*" />
                                </div>
                                <button type="submit" className="form-submit btn-blocker" style={{ borderRadius: 'unset' }}>Đăng ký</button>
                                <p style={{ fontSize: '16px', margin: '10px 0', textAlign: 'center' }}>Bạn đã có tài khoản? <Link to="/login" style={{ color: 'black', fontWeight: 'bold' }}>Đăng nhập</Link></p>
                            </form>
